...
semantic_import = "import { BED_SEMANTIC_LANDINGS } from '@/lib/bed-semantic-core';"
...
if "size: rawRouteSize" not in text:
    text = text.replace(
...
if "Популярні категорії" not in text:
    if needle not in text:
        raise SystemExit('semantic insertion point not found')
    text = text.replace(needle, needle + semantic_block, 1)
