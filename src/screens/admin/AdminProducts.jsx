...
function normalizeProduct(product, fallbackIndex = 0) {
  ...
  return {
    ...
    installment_months: Number(product.installment_months || 6),
    installment_provider: product.installment_provider || '',
    installment_monthly_from: Number(product.installment_monthly_from || 0) || null,
...
function ProductEditor({ product, onClose, onSave }) {
  ...
  const installment = installmentConfig(draft, Number(draft.price_current || draft.price || 0));
  ...
  const save = async () => {
    ...
    if (draft.installment_enabled && (!draft.installment_months || Number(draft.installment_months) < 2)) return setValidation('Для розстрочки вкажіть щонайменше 2 платежі.');
  ...
  return <div className="fixed inset-0 z-[80] bg-[#342112]/45 flex justify-end"><div className="w-full max-w-5xl h-full bg-[#FAF7F2] overflow-y-auto shadow-2xl">
    ...
    <div className="grid lg:grid-cols-[1fr_290px] gap-7 px-6 py-7"><div className="space-y-5">
      ...
      {tab === 'price' && <>
        ...
        <div className="mt-6 border-t border-[#342112]/10 pt-5">
          <label className="flex items-center gap-3 bg-white border border-[#342112]/10 p-4"><input type="checkbox" checked={draft.installment_enabled !== false} onChange={(e)=>set('installment_enabled',e.target.checked)} /><span className="text-sm"><strong>Показувати розстрочку / оплату частинами</strong><span className="block text-xs text-[#937C68] mt-1">Виводиться біля ціни на картці товару та PDP.</span></span></label>
          {draft.installment_enabled !== false && <><div className="grid md:grid-cols-2 gap-5 mt-5"><Field label="Кількість платежів" type="number" value={draft.installment_months || 6} onChange={(v)=>set('installment_months',Math.max(2,Math.min(24,v||6)))} /><Field label="Банк / партнер" value={draft.installment_provider} onChange={(v)=>set('installment_provider',v)} placeholder="Monobank / PrivatBank" /><Field label="Від, ₴/міс — вручну (optional)" type="number" value={draft.installment_monthly_from} onChange={(v)=>set('installment_monthly_from',v)} /><Field label="Кастомний текст (optional)" value={draft.installment_text} onChange={(v)=>set('installment_text',v)} placeholder="Оплата частинами до 10 платежів" /></div><div className="mt-4 p-4 bg-[#F5E4D1]/55 border border-[#342112]/10"><div className="text-[10px] uppercase tracking-[.16em] text-[#937C68]">Preview</div><div className="text-sm text-[#342112] mt-1 font-medium">{installmentLabel(draft, Number(draft.price_current || 0))}</div><div className="text-xs text-[#755A44] mt-1">Розрахунок: {installment.monthly.toLocaleString('uk-UA')} ₴ × {installment.months} платежів. Якщо партнер не вказаний — сайт не називає конкретний банк.</div></div></>}
        </div>
      </>}
