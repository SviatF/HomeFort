...
function normalizeProduct(product, fallbackIndex = 0) {
  ...
  return {
    ...
    monobank_enabled: product.monobank_enabled ?? true,
    monobank_months: Number(product.monobank_months || product.installment_months || 6),
    monobank_monthly_from: Number(product.monobank_monthly_from || 0) || null,
    privatbank_enabled: product.privatbank_enabled ?? true,
    privatbank_months: Number(product.privatbank_months || product.installment_months || 6),
    privatbank_monthly_from: Number(product.privatbank_monthly_from || 0) || null,
    ...product,
...
function ProductEditor({ product, onClose, onSave }) {
  ...
  const installmentOptions = bankInstallmentOptions(draft, Number(draft.price_current || draft.price || 0));
  ...
  const save = async () => {
    ...
    if (draft.installment_enabled !== false && draft.monobank_enabled && Number(draft.monobank_months || 0) < 2) return setValidation('Для monobank вкажіть щонайменше 2 платежі.');
    if (draft.installment_enabled !== false && draft.privatbank_enabled && Number(draft.privatbank_months || 0) < 2) return setValidation('Для ПриватБанк вкажіть щонайменше 2 платежі.');
  ...
  return <div className="fixed inset-0 z-[80] bg-[#342112]/45 flex justify-end"><div className="w-full max-w-5xl h-full bg-[#FAF7F2] overflow-y-auto shadow-2xl">
    ...
    <div className="grid lg:grid-cols-[1fr_290px] gap-7 px-6 py-7"><div className="space-y-5">
      ...
      {tab === 'price' && <>
        <div className="grid md:grid-cols-2 gap-5"><Field label="Актуальна ціна, ₴" type="number" value={draft.price_current} onChange={(v)=>{set('price_current',v);set('price',v);}} /><Field label="Стара ціна, ₴" type="number" value={draft.price_old} onChange={(v)=>{set('price_old',v);set('oldPrice',v);}} /><Field label="Підпис акції" value={draft.discount_label} onChange={(v)=>set('discount_label',v)} placeholder="Акція −15% / Розпродаж серії LUX" /><Field label="Акція діє до" type="datetime-local" value={draft.discount_valid_until} onChange={(v)=>set('discount_valid_until',v)} /></div>
        {draft.price_old > draft.price_current && <div className="p-4 bg-[#C8643B]/10 border border-[#C8643B]/20 text-sm text-[#8B4229]">На сайті буде показано автоматичну знижку <strong>−{Math.round((1 - draft.price_current / draft.price_old) * 100)}%</strong>. Після дедлайну стара ціна, badge і countdown автоматично зникнуть.</div>}

        <div className="mt-6 border-t border-[#342112]/10 pt-5 space-y-4">
          <label className="flex items-center gap-3 bg-white border border-[#342112]/10 p-4"><input type="checkbox" checked={draft.installment_enabled !== false} onChange={(e)=>set('installment_enabled',e.target.checked)} /><span className="text-sm"><strong>Показувати розстрочку на сторінці товару</strong><span className="block text-xs text-[#937C68] mt-1">У каталозі розстрочка не показується. На PDP — окремі premium-блоки банків біля ціни.</span></span></label>
          {draft.installment_enabled !== false && <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-white border border-[#342112]/10 p-5">
              <div className="flex items-center justify-between gap-3 mb-4"><div className="inline-flex px-3 h-9 items-center rounded-lg bg-black text-white font-semibold text-sm">mono</div><label className="flex items-center gap-2 text-xs text-[#342112]"><input type="checkbox" checked={draft.monobank_enabled !== false} onChange={(e)=>set('monobank_enabled',e.target.checked)} /> Увімкнено</label></div>
              <div className="grid gap-4"><Field label="Кількість платежів" type="number" value={draft.monobank_months || 6} onChange={(v)=>set('monobank_months',Math.max(2,Math.min(24,v||6)))} /><Field label="Від, ₴/міс — вручну (optional)" type="number" value={draft.monobank_monthly_from} onChange={(v)=>set('monobank_monthly_from',v)} /></div>
              {draft.monobank_enabled !== false && <div className="mt-4 text-xs text-[#755A44]">Preview: <strong className="text-[#342112]">від {(installmentOptions.find(x=>x.id==='monobank')?.monthly || 0).toLocaleString('uk-UA')} ₴/міс</strong> · до {draft.monobank_months || 6} платежів</div>}
            </div>
            <div className="bg-white border border-[#342112]/10 p-5">
              <div className="flex items-center justify-between gap-3 mb-4"><div className="inline-flex px-3 h-9 items-center rounded-lg bg-[#69A82F] text-white font-semibold text-xs">ПриватБанк</div><label className="flex items-center gap-2 text-xs text-[#342112]"><input type="checkbox" checked={draft.privatbank_enabled !== false} onChange={(e)=>set('privatbank_enabled',e.target.checked)} /> Увімкнено</label></div>
              <div className="grid gap-4"><Field label="Кількість платежів" type="number" value={draft.privatbank_months || 6} onChange={(v)=>set('privatbank_months',Math.max(2,Math.min(24,v||6)))} /><Field label="Від, ₴/міс — вручну (optional)" type="number" value={draft.privatbank_monthly_from} onChange={(v)=>set('privatbank_monthly_from',v)} /></div>
              {draft.privatbank_enabled !== false && <div className="mt-4 text-xs text-[#755A44]">Preview: <strong className="text-[#342112]">від {(installmentOptions.find(x=>x.id==='privatbank')?.monthly || 0).toLocaleString('uk-UA')} ₴/міс</strong> · до {draft.privatbank_months || 6} платежів</div>}
            </div>
          </div>}
          <p className="text-xs text-[#755A44]">Якщо поле «від ₴/міс» порожнє, сума рахується автоматично як актуальна ціна / кількість платежів. Фактичний ліміт і доступність визначає банк.</p>
        </div>
      </>}
