...
const cols = [
  ...
  { title: 'Покупцям', links: ['Доставка та оплата', 'Гарантія', 'Відстежити замовлення', 'Trade-In', 'Шоурум Одеса', 'Конфігуратор', 'Обране', 'Порівняння'] },
      ...
      <div className="mx-auto max-w-[1440px] px-6 lg:px-12">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 pb-16 border-b border-milk/15">
          <div className="lg:col-span-4">
            <div className="mb-6">
              ...
              <p className="text-[10px] tracking-[0.28em] mt-2 text-milk/60 font-medium">
                {settings?.brandDescriptor || 'Ліжка та матраци власного виробництва'}
              </p>
            </div>
            <p className="text-milk/75 leading-relaxed max-w-sm">
              Ліжка, матраци та текстиль нового покоління для вашого комфорту. Власне виробництво, продумані рішення для сну.
            </p>
            ...
            {cols.map((c) => (
              <div key={c.title}>
                ...
                <ul className="space-y-2.5">
                  {c.links.map((l) => (
                    <li key={l}>
                      <Link to={linkPaths[l] || '/'} className="text-sm text-milk/70 hover:text-milk transition-colors">{l}</Link>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 pt-8 text-milk/55 text-sm">
          ...
          <div className="flex items-center gap-6">
            <a href="https://instagram.com" target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-milk transition-colors"><Instagram className="w-4 h-4" strokeWidth={1.5} /> Instagram</a>
            <Link to="/" className="hover:text-milk transition-colors">Політика конфіденційності</Link>
            <Link to="/" className="hover:text-milk transition-colors">Умови</Link>
          </div>
        </div>
      </div>
    </footer>
