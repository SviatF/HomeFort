...
const catalog = [
  {
    title: 'Ліжка',
    items: ["М'які", 'З підйомним механізмом', 'Односпальні', 'Двоспальні', 'Дитячі', 'Преміум'],
  },
  { title: 'Матраци', items: ['Преміум', 'Середньої жорсткості', 'Жорсткі', 'Анатомічні'] },
  { title: 'Текстиль', items: ['Наматрацники', 'Подушки', 'Ковдри', 'Постільна білизна'] },
  { title: 'Дитячі матраци', items: ['Від 3 років', 'Від 7 років', 'Підліткові'] },
];
...
export default function Header({ dark = false }) {
  ...
  const d = dark;
  const bar = d
    ? (scrolled ? 'bg-espresso' : 'bg-espresso/92')
    : (scrolled ? 'bg-milk' : 'bg-milk/92');
  ...
  return (
    <>
      <header
        className={`fixed top-0 inset-x-0 z-50 transition-all duration-500 backdrop-blur-md border-b ${bar} ${
          scrolled ? `${borderScroll} shadow-card` : borderIdle
        }`}
      >
        <div className="mx-auto max-w-[1440px] px-6 lg:px-12">
          <div className="flex items-center justify-between h-[78px]">
            ...
            <Link to="/" className="flex flex-col leading-none select-none">
              <span className={`font-heading text-[26px] tracking-[0.22em] ${logo} font-medium`}>
                DOMERA
              </span>
              <span className={`text-[9px] tracking-[0.36em] mt-1 ${sub} font-bold`}>
                HOME TEXTILE &amp; LIVING
              </span>
            </Link>
            ...
            <nav className="hidden lg:flex items-center gap-8">
              <div onMouseEnter={() => setMega(true)}>
                <button className={`flex items-center gap-1 text-[12px] tracking-[0.16em] uppercase font-medium ${nav} ${hover} transition-colors`}>
                  Каталог <ChevronDown className="w-3 h-3" />
                </button>
              </div>
              {navLinks.map((l) => (
                <Link
                  key={l.label}
                  to={l.to}
                  className={`text-[12px] tracking-[0.16em] uppercase font-medium ${nav} ${hover} transition-colors`}
                >
                  {l.label}
                </Link>
              ))}
        ...
        <div
          ...
          className={`absolute left-0 right-0 top-full transition-all duration-300 origin-top ${
            mega ? 'opacity-100 visible' : 'opacity-0 invisible'
          }`}
        >
          <div className={`${megaBg} border-t ${megaBorder} shadow-elevated`}>
            <div className="mx-auto max-w-[1440px] px-6 lg:px-12 py-12 grid grid-cols-4 gap-10">
              {catalog.map((c, i) => (
                <div key={c.title}>
                  <span className={`text-[10px] tracking-[0.32em] ${megaNum} uppercase block mb-4 font-medium`}>
                    0{i + 1}
                  </span>
                  ...
                  <ul className="space-y-2.5">
                    {c.items.map((it) => (
                      <li key={it}>
                        <Link to="/catalog/beds" className={`text-sm ${megaItem} transition-colors`}>
                          {it}
                        </Link>
                      </li>
                    ))}
      ...
      >
        ...
        <div
          className={`absolute right-0 top-0 h-full w-[88%] max-w-[420px] ${mobileBg} transition-transform duration-500 ${
        ...
        >
          <div className={`flex items-center justify-between px-6 h-[78px] border-b ${mobileBorder}`}>
            ...
            <button aria-label="Закрити" onClick={() => setMobile(false)}>
              <X className={`w-6 h-6 ${logo}`} strokeWidth={1.5} />
            </button>
          </div>
          <div className="px-6 py-8 overflow-y-auto h-[calc(100%-78px)]">
              ...
              {catalog.map((c) => (
                <div key={c.title} className={`py-2 border-b ${mobileBorder}`}>
                  ...
                  <ul className="mt-2 space-y-1.5">
                    {c.items.map((it) => (
                      <li key={it}>
                        <Link to="/catalog/beds" onClick={() => setMobile(false)} className={`text-sm ${mobileSub}`}>{it}</Link>
                      </li>
                    ))}
                  </ul>
                </div>
            ...
            <nav className="space-y-3">
              {navLinks.map((l) => (
                <Link
                  ...
                  className={`block text-sm tracking-[0.16em] uppercase ${nav} font-medium`}
                >
                  {l.label}
                </Link>
              ))}
            ...
            <div className={`mt-8 pt-6 border-t ${mobileBorder} space-y-3 ${mobileSub}`}>
              <a href="tel:+380441234567" className="flex items-center gap-2 text-sm"><Phone className="w-4 h-4" strokeWidth={1.5} /> +380 44 123 45 67</a>
              <p className="text-sm">Одеса, вул. Текстильна, 12</p>
            </div>
          </div>
        </div>
      </div>
