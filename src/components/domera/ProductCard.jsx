...
function AvailabilityBadge({ children }) {
  return (
    <span className="hidden min-h-[23px] items-center rounded-[7px] border border-[#ddd0c4]/75 bg-[#fffdf9]/94 px-2 py-0.5 text-[9px] font-normal leading-none tracking-[0.01em] text-[#665b52] shadow-[0_1px_7px_rgba(53,37,26,0.03)] backdrop-blur-md md:inline-flex md:text-[10px]">
      {children}
    </span>
  );
}
    ...
    <article className={`product-card-shell group relative border border-[#e8e0d7] bg-[#fffdfa] shadow-[0_8px_28px_rgba(48,34,24,0.045)] transition-[transform,box-shadow,border-color] duration-500 ease-out md:hover:-translate-y-[3px] md:hover:border-[#ded2c6] md:hover:shadow-[0_15px_38px_rgba(48,34,24,0.075)] ${dark ? 'text-milk' : 'text-espresso'}`}>
      <div className="product-card-media relative overflow-hidden">
        <Link to={`/product/${product.slug}`} onClick={() => { rememberScroll(); track('select_item', { items: [buildItem({ ...product, price })] }); }} className="block h-full" data-tap-target="true">
          {activeImage ? (
            <ProductImage src={activeImage} alt={product.imageAlt || product.name} sizes="(max-width: 767px) calc(100vw - 40px), (max-width: 1279px) 33vw, 28vw" quality={60} className="h-full w-full transition-transform duration-700 ease-out md:group-hover:scale-[1.022] motion-reduce:transform-none" onError={handleImageError} />
          ) : (
        ...
        {hasGallery && <>
          <button type="button" aria-label="Попереднє фото" onClick={(e) => { e.preventDefault(); e.stopPropagation(); goToImage(-1); }} className="ui-radius-sm absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center border border-[#e8dfd6]/75 bg-[#fffdf9]/88 text-espresso shadow-[0_2px_10px_rgba(52,36,24,0.035)] backdrop-blur-md transition-all duration-300 md:left-2.5 md:h-9 md:w-9 md:opacity-0 md:group-hover:opacity-100"><ChevronLeft className="h-3.5 w-3.5" strokeWidth={1.4} /></button>
          <button type="button" aria-label="Наступне фото" onClick={(e) => { e.preventDefault(); e.stopPropagation(); goToImage(1); }} className="ui-radius-sm absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center border border-[#e8dfd6]/75 bg-[#fffdf9]/88 text-espresso shadow-[0_2px_10px_rgba(52,36,24,0.035)] backdrop-blur-md transition-all duration-300 md:right-2.5 md:h-9 md:w-9 md:opacity-0 md:group-hover:opacity-100"><ChevronRight className="h-3.5 w-3.5" strokeWidth={1.4} /></button>
        </>}
      </div>

      <div className="p-4 sm:p-5">
        <Link to={`/product/${product.slug}`} className="block" data-tap-target="true">
          <h3 className="font-heading text-[24px] leading-[1.05] text-espresso md:text-[25px]">{title}</h3>
        </Link>
        ...
        <div className="mt-3 flex min-h-[32px] flex-wrap items-baseline gap-x-2.5 gap-y-0.5">
          ...
          {discounted && previousPrice > price && <span className="text-[13px] text-mocha/80 line-through">{Number(previousPrice).toLocaleString('uk-UA')} ₴</span>}
        </div>

        {savings > 0 && (
          <div className="mt-2.5 inline-flex min-h-[27px] items-center rounded-[8px] border border-[#d5e5d1] bg-[#eef5eb] px-2.5 py-1.5 text-[11px] font-semibold leading-none tracking-[0.035em] text-[#456b49] md:text-[11px]">
            Економія {savings.toLocaleString('uk-UA')} ₴
          </div>
        )}

        {swatches.length > 0 && <div className="mt-3 hidden sm:block"><p className="mb-2 text-[13px] text-mocha">Тканини / кольори</p><div className="flex gap-2" aria-label="Доступні кольори тканини">{swatches.map((s, i) => <span key={`${s.name}-${i}`} title={s.name} className="fabric-dot" style={s.image ? { backgroundImage: `url(${s.image})`, backgroundSize: 'cover' } : { background: s.color }} />)}</div></div>}

        <Link to={`/product/${product.slug}`} onClick={() => { rememberScroll(); track('select_item', { items: [buildItem({ ...product, price })] }); }} className="ui-action ui-radius-sm group/cta mt-4 inline-flex min-h-[50px] w-full items-center justify-center gap-2.5 px-4 py-3.5 text-center text-[13px] font-medium uppercase leading-[1.2] tracking-[0.085em] transition-all duration-300 md:hover:shadow-[0_7px_18px_rgba(47,31,20,0.16)]" data-tap-target="true">
          <span>Обрати комплектацію</span><ArrowUpRight className="h-3.5 w-3.5 shrink-0 opacity-75 transition-transform duration-300 group-hover/cta:translate-x-0.5 group-hover/cta:-translate-y-0.5" strokeWidth={1.35} />
        </Link>
      </div>
    </article>
  );
